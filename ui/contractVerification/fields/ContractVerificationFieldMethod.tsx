...
const ContractVerificationFieldMethod = ({
  control,
  isDisabled,
  methods,
}: Props) => {
  ...
  const renderPopoverListItem = React.useCallback(
    (method: SmartContractVerificationMethod) => {
      switch (method) {
        ...
        case 'vyper-code':
          return (
            <ListItem key={ method }>Verification of Vyper contract.</ListItem>
          );
    ...
    <>
      <div>
        <chakra.span fontWeight={ 500 } fontSize="lg" fontFamily="heading">
          Currently, EthDA explorer supports { methods.length } contract
          ...
          <Portal>
            ...
            >
              ...
              <PopoverBody color="white">
                <DarkMode>
                  <span>
                    Currently, EthDA explorer supports { methods.length } methods:
                  </span>
                  <OrderedList>
